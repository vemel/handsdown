...
class BaseGenerator:
    ...
    common_templates_path = Path("common")
    templates_path = Path("mkdocs")
    ...
    def __init__(
        ...
        if not self._output_path.exists():
            self._logger.info(f"Creating folder {print_path(self._output_path)}")
    ...
    def cleanup_old_docs(self) -> None:
        ...
        for doc_path in PathFinder(self._output_path).glob("**/*.md"):
            ...
            self._logger.info(f"Deleting orphaned doc file {print_path(doc_path)}")
            doc_path.unlink()
        ...
        for orphaned_dir in orphaned_dirs:
            self._logger.info(f"Deleting orphaned directory {print_path(orphaned_dir)}")
            orphaned_dir.rmdir()
    ...
    def generate_index(self) -> None:
        ...
        """
        self._logger.debug(f"Generating {print_path(self.md_index.path)}")
        ...
        if self._write_changed(md_document.path, content):
            self._logger.info(f"Updated index {print_path(md_document.path)}")
    ...
    def _write_changed(self, path: Path, content: str) -> bool:
        ...
        path.write_text(content, self._encoding)
        return True

...
class Generator:
    ...
    def _generate_doc(self, module_record, md_document):
        ...
        self._render_docstring(record=module_record, md_document=md_document)
    ...
    def _replace_short_links(self, module_record, md_document):
        # type: (ModuleRecord, MDDocument) -> None
        records = [i[-1] for i in module_record.iter_children()]
        if not records:
            return
    ...
    def _generate_module_doc_lines(self, module_record, md_document):
        # type: (ModuleRecord, MDDocument) -> None
        for records in module_record.iter_children():
            record = records[-1]
            ...
            self._render_docstring(record=record, md_document=md_document)

    def _render_docstring(self, record, md_document):
        # type: (NodeRecord, MDDocument) -> None
        """
        ...
        for import_string in record.related_import_strings:

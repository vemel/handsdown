...
class Generator:
    ...
    def generate_modules(self) -> None:
        ...
        md_document = self.md_modules
        content = self._jinja_manager.render(
            template_path=Path("mkdocs") / "modules.md.jinja2",
            md_document=md_document,
            generator=self,
        )
        md_document.path_finder.mkdir()
        if NicePath(md_document.path).write_changed(content, encoding=self._encoding):
            self._logger.info(f"Updated modules {NicePath(md_document.path)}")
